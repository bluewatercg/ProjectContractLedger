...
REM 检查Docker文件
echo 5. 检查Docker配置
if exist "tools\docker\Dockerfile" (
    echo Dockerfile存在: ✅ (tools\docker\Dockerfile)
    for /f %%i in ('find /c /v "" ^< tools\docker\Dockerfile') do echo Dockerfile大小: %%i 行
) else (
    echo Dockerfile不存在: ❌ (应在 tools\docker\Dockerfile)
)

if exist "tools\docker\docker-compose.yml" (
    echo docker-compose.yml存在: ✅ (tools\docker\docker-compose.yml)
) else (
    echo docker-compose.yml不存在: ❌ (应在 tools\docker\docker-compose.yml)
)
echo.
