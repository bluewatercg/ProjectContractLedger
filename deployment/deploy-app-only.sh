...
# 客户合同管理系统 - 仅应用容器部署脚本
# 适用于已有MySQL、Redis和反向代理服务的部署场景
# 使用方法: ./deploy-app-only.sh [选项]
...
check_environment() {
    ...
    if [ ! -f "${SCRIPT_DIR}/${ENV_FILE}" ]; then
        ...
        log_info "请复制 .env.app-only.template 为 ${ENV_FILE} 并填写配置"
        exit 1
    fi
...
check_ports() {
    ...
    API_PORT=${APP_API_PORT:-8080}
    
    if netstat -tlnp 2>/dev/null | grep -q ":${HTTP_PORT} "; then
        log_warning "端口 ${HTTP_PORT} 已被占用"
        netstat -tlnp 2>/dev/null | grep ":${HTTP_PORT} "
    fi
...
check_service_health() {
    ...
    for i in {1..10}; do
        if curl -f http://localhost:${HTTP_PORT}/health >/dev/null 2>&1; then
            log_success "前端服务健康检查通过 (端口 ${HTTP_PORT})"
            break
        fi
        if [ $i -eq 10 ]; then
    ...
    for i in {1..10}; do
        if curl -f http://localhost:${API_PORT}/api/health >/dev/null 2>&1; then
            log_success "API服务健康检查通过 (端口 ${API_PORT})"
            return 0
        fi
        if [ $i -eq 10 ]; then
...
show_status() {
    ...
    netstat -tlnp 2>/dev/null | grep -E ":${HTTP_PORT}|:${API_PORT}" || echo "未找到监听端口"
    
    echo
    log_info "系统资源使用情况:"
    docker stats --no-stream --format "table {{.Container}}\t{{.CPUPerc}}\t{{.MemUsage}}\t{{.NetIO}}\t{{.BlockIO}}" | grep contract-ledger || echo "未找到容器"
}
...
show_connection_info() {
    ...
    echo
    log_info "应用连接信息:"
    echo "前端服务: http://localhost:${HTTP_PORT}"
    echo "API服务:  http://localhost:${API_PORT}"
    echo "健康检查: http://localhost:${HTTP_PORT}/health"
    echo "API健康检查: http://localhost:${API_PORT}/api/health"
    echo
    log_info "请确保您的反向代理已配置指向这些端口"
}

# 显示帮助信息
show_help() {
    echo "客户合同管理系统 - 仅应用容器部署脚本"
    echo
    echo "使用方法: $0 [选项]"
    echo
    echo "选项:"
