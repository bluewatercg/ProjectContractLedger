...
async function generatePasswordHashes() {
  ...
  console.log('验证哈希值:');
  console.log(
    `Admin密码验证: ${await bcrypt.compare(adminPassword, adminHash)}`
  );
  ...
  console.log(
    `('admin', 'admin@example.com', '${adminHash}', '系统管理员', 'admin', 'active');`
  );
  console.log('');
  ...
  console.log(
    `('testuser', 'test@example.com', '${userHash}', '测试用户', 'user', 'active');`
  );
}
