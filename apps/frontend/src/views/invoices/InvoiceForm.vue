            ...
            >
              ...
              >
                <el-option
                  ...
                  :key="plan.id"
                  :label="`${plan.phase_name}（计划 ¥${formatPlanAmount(
                    plan.planned_amount
                  )}，已开 ¥${formatPlanAmount(
                    plan.actual_invoiced_amount || 0
                  )}）`"
                  :value="plan.id"
                />
              </el-select>
...
const fetchInvoice = async () => {
  ...
  try {
    ...
    if (response.success && response.data) {
      ...
      // 检查是否已作废（后端终态：status === 'cancelled'）
      if (invoiceData.status === "cancelled") {
        isVoided.value = true;
        ElMessage.warning("该发票已作废，无法编辑");
        router.replace(`/invoices/${invoiceId.value}`);
        return;
      }

      // 仅取 DTO 允许字段，防止伪造 id/kit_id/void/bad_debt
      form.contract_id = invoiceData.contract_id || 0;
      form.plan_id = invoiceData.plan_id ?? undefined;
      form.amount = Number(invoiceData.amount || 0);
      form.tax_rate = Number(invoiceData.tax_rate || 0);
      form.issue_date = parseDate(invoiceData.issue_date) || "";
      form.due_date = parseDate(invoiceData.due_date) || "";
      form.description = invoiceData.description || "";
      form.notes = invoiceData.notes || "";
...
const handleSubmit = async () => {
  if (!formRef.value || isVoided.value) return;
