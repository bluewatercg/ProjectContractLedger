  ...
  <div class="page-container">
    <div class="page-header">
      ...
      <div>
        ...
        <template v-if="showRenewalActions">
          <el-button type="success" @click="handleConfirmRenew"
            >确认续签</el-button
          >
          <el-button type="danger" @click="showNonRenewDialog = true"
            >确认不续签</el-button
          >
        </template>
      </div>
    ...
    <div v-loading="loading">
      <el-descriptions v-if="contract" :column="2" border>
        ...
        <el-descriptions-item label="合同金额"
          >¥{{ formatCurrency(contract.total_amount) }}</el-descriptions-item
        >
            ...
            <div v-if="contract.invoices && contract.invoices.length > 0">
              ...
              <div class="stats-container">
                ...
                <div class="stat-card stat-card-purple">
                  ...
                  <div
                    v-if="invoiceStats.uninvoicedAmount > 0"
                    class="stat-action"
                  >
                    <el-button
                      type="primary"
                ...
                <el-table-column label="收款情况" min-width="300">
                  <template #default="scope">
                    <div>
                      ...
                      <div
                        v-if="
                          scope.row.payments && scope.row.payments.length > 0
                        "
                      >
                        <div
    ...
    >
      ...
      <template #footer>
        ...
        <el-button
          type="danger"
          :loading="nonRenewLoading"
          @click="handleConfirmNonRenew"
        >
          确认不续签
        </el-button>
      </template>
...
const showRenewalActions = computed(() => {
  ...
  const isExpired =
    contract.value.end_date && new Date(contract.value.end_date) < new Date();
  return (
    contract.value.status === "active" &&
    contract.value.is_renewable &&
    isExpired
  );
});
...
const invoiceStats = computed(() => {
  if (!contract.value?.invoices || !Array.isArray(contract.value.invoices)) {
    return {
      ...
      paidAmount: 0,
      unpaidAmount: 0,
      uninvoicedAmount: contract.value
        ? safeNumber(contract.value.total_amount)
        : 0,
    };
  }
