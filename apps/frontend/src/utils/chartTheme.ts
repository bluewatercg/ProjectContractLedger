...
export const chartColors = {
  ...
  blue: ["#533AFD", "#665EFD", "#B9B9F9", "#DDD8FF"],
...
export const globalChartConfig: Partial<EChartsOption> = {
  ...
  textStyle: {
    fontFamily:
      'IBM Plex Sans, -apple-system, BlinkMacSystemFont, "Segoe UI", "PingFang SC", "Microsoft YaHei", sans-serif',
    fontSize: 14,
    color: "#273951",
  },
  animation: true,
      ...
      itemStyle: {
        ...
        color: createGradient(
          chartColors.gradient[
            Object.keys(chartColors.gradient)[
              index % 4
            ] as keyof typeof chartColors.gradient
          ],
        ),
      },
    })),
...
// 创建现代饼图
export function createModernPieChart(
...
// 创建环形进度图
export function createRingProgressChart(data: {
  name: string;
  value: number;
  total: number;
}): EChartsOption {
